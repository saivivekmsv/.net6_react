...
export const DifferentInvestmentElection = (props) => {
  ...
  useDeepEffect(() => {
    let defaults = !isEmpty(
      additionalAutoEnrollmentData.additionalAutoEnrollmentDifferentInvestmentElection
    )
      ? [
          ...additionalAutoEnrollmentData?.additionalAutoEnrollmentDifferentInvestmentElection,
        ]
    ...
    if (values.sameInvestmentBasedOn === 1) {
      setSaveInvestments({ 1: additionalAutoEnrollmentData.savedInvestments });
      setAddedInvestments({ 1: additionalAutoEnrollmentData.savedInvestments });
      if (
        !isEmpty(
          additionalAutoEnrollmentData.additionalAutoEnrollmentDifferentInvestmentElection
        )
      ...
      setSaveInvestments({ 2: additionalAutoEnrollmentData.savedInvestments });
      setAddedInvestments({ 2: additionalAutoEnrollmentData.savedInvestments });
      if (
        !isEmpty(
          additionalAutoEnrollmentData.additionalAutoEnrollmentDifferentInvestmentElection
        )
      ...
      setSaveInvestments({ 3: additionalAutoEnrollmentData.savedInvestments });
      setAddedInvestments({ 3: additionalAutoEnrollmentData.savedInvestments });
      if (
        !isEmpty(
          additionalAutoEnrollmentData.additionalAutoEnrollmentDifferentInvestmentElection
        )
  ...
  //console.log(autoEnrollment.additionalAutoEnrollmentDifferentInvestmentElection.map(x=>x.map(y=>({"from":y[0].birthDateFrom,'to':y[0].birthDateTo})).filter(x=>x.from!=null)).flatMap(x=>x),'investmentName')
  ...
  const deletefunction = (name, index, sourceInd, cellIndex, sIndex) => {
    ...
    //console.log( values.additionalAutoEnrollmentDifferentInvestmentElection[sourceInd][index-1][sIndex],index)
    for (var i = 0; i <= index; i++) {
      if (
        values.additionalAutoEnrollmentDifferentInvestmentElection[sourceInd] !=
          null &&
        values.additionalAutoEnrollmentDifferentInvestmentElection[sourceInd][
          i
        ] != null &&
        ...
        values.additionalAutoEnrollmentDifferentInvestmentElection[sourceInd][
          i
        ] = values.additionalAutoEnrollmentDifferentInvestmentElection[
          sourceInd
        ][i].filter((x) => x.investmentName != name);
      }
    }
  };
  ...
  const onDeleteClick = (name, index, sourceInd, cellIndex, sIndex) => {
    ...
    //     )
    //   : deletefunction(name);
    //console.log( values.additionalAutoEnrollmentDifferentInvestmentElection[sourceInd][index])
  ...
  const Invsum = (index, cellIndex) => {
    let sum1 = 0;
    if (
      !isEmpty(values.additionalAutoEnrollmentDifferentInvestmentElection) &&
      !isEmpty(values.additionalAutoEnrollmentDifferentInvestmentElection) &&
      !isEmpty(
        values.additionalAutoEnrollmentDifferentInvestmentElection[index]
      ) &&
      !isEmpty(
        values.additionalAutoEnrollmentDifferentInvestmentElection[index][
          cellIndex
      ...
      values.additionalAutoEnrollmentDifferentInvestmentElection[index][
        cellIndex
      ].map((x) => (sum1 = sum1 + x.investmentPercentage));
    return sum1;
  };
  const AddName = (
    e,
    ...
    console.log("in");
    // console.log(values.additionalAutoEnrollmentDifferentInvestmentElection[index][cellIndex][
    //   sourceInd
    // ])
    if (
      !isEmpty(values.additionalAutoEnrollmentDifferentInvestmentElection) &&
      ...
      values.additionalAutoEnrollmentDifferentInvestmentElection[index][
        cellIndex
      ][sourceInd].investmentName = source;
      values.additionalAutoEnrollmentDifferentInvestmentElection[index][
        cellIndex
      ][sourceInd].investmentId = investmentId;
      values.additionalAutoEnrollmentDifferentInvestmentElection[index][
        cellIndex
      ][sourceInd].sourceName = sourceName;
      values.additionalAutoEnrollmentDifferentInvestmentElection[index][
        cellIndex
      ][sourceInd].sourceId = sourceId;
      const x = sName.split("-");
      if (category === 1) {
        values.additionalAutoEnrollmentDifferentInvestmentElection[index][
          cellIndex
        ][sourceInd].ageFrom = x[0];
        values.additionalAutoEnrollmentDifferentInvestmentElection[index][
          cellIndex
        ][sourceInd].ageTo = x[1];
      } else if (category === 2) {
        values.additionalAutoEnrollmentDifferentInvestmentElection[index][
          cellIndex
        ][sourceInd].birthDateFrom = x[0];
        values.additionalAutoEnrollmentDifferentInvestmentElection[index][
          cellIndex
        ][sourceInd].birthDateTo = x[1];
      } else if (category === 3) {
        values.additionalAutoEnrollmentDifferentInvestmentElection[index][
          cellIndex
        ][sourceInd].retirementDateFrom = x[0];
        values.additionalAutoEnrollmentDifferentInvestmentElection[index][
          cellIndex
        ][sourceInd].retirementDateTo = x[1];
      }
      // }
    }
    handleChange(e);
    ...
    <div>
      <Field
        ...
        onChange={(value) => {
          ...
          // // setAddedInvestments({});
          // // setTotalInvestments({});
          // console.log(values.additionalAutoEnrollmentDifferentInvestmentElection);
                                ...
                                return (
                                  <Table.Tr key={sourceInd}>
                                    {ageTable.map((item, cellIndex) => {
                                      // console.log(investment)
                                      const fieldName = `additionalAutoEnrollmentDifferentInvestmentElection[${index}][${
                                        cellIndex - 1
