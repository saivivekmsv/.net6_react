import { filter, get, uniq, isEmpty } from "lodash";
...
const PayrollCompensationDetails = ({
  fields,
  data,
  paydate,
  pendingSubmission,
  ...
  const check = (errorMsg) => {
    const errorId = error.filter((_) => _.messageDescCode == errorMsg);
    if (!isEmpty(errorId)) {
      setCompError([...compError, errorId[0].id]);
      compensationError([...compError, errorId[0].id]);
    }
  };
            ...
            <Table className="compensationDetails">
              ...
              <Table.Tbody>
                {filteredValues.map((compensation, index) => {
                  ...
                  const isError =
                    get(errors, `compensations[${index}].ytdAmount`) !=
                    undefined
                      ? true
                      : false;
                        ...
                        {compensation.compensationType
                          ? columns.map((item, cellIndex) => {
                              const getContent = () => {
                                ...
                                if (
                                  item.columnName === "removebutton" &&
                                  values.modeOfCompensation === 1 &&
                                  isError
                                ) {
                                  {
                                    /* return [
