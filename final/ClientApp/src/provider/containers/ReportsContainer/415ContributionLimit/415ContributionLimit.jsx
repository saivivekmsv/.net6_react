...
class KendoGridDecimalCell extends React.Component {
  render() {
...
const columns = [
  ...
  },
  {
    value: 9,
    label: "Non Elective",
    className: "column-nonElective",
    columnName: "nonElective",
    field: "nonElective",
    dataType: "amount",
    width: 144,
    field1: "nonElectiveFrom",
    field2: "nonElectiveTo",
  },
  {
    value: 10,
    ...
    width: 233,
    dataType: "amount",
    field1: "totalContributionFrom",
    field2: "totalContributionTo",
  },
];
...
const ContributionLimit = (props) => {
  ...
  const formName = manageReportsFormNames.CONTRIBUTION_LIMITS;
  ...
  const [contribution415Table, setContribution415Table] = useState([]);
  ...
  const apiCall = (data) => {
    setDataLoading(true);
    get415ReportInfo(data)
      .then((response) => {
        setDataLoading(false);
        setContribution415Table(response.report415Contributions);
  ...
  const onSubmit = (values) => {
    ...
    if (values.companyId !== null) {
      ...
      var filters = {
        ...
        payDateFrom: values.payDateFrom,
        payDateTo: values.payDateTo,
        amountFrom: values.amountFrom ? parseFloat(values.amountFrom) : 0,
        amountTo: values.amountTo ? parseFloat(values.amountTo) : 0,
        from: 1,
        count: 20,
        innerFilters: null,
    ...
    if (
      ...
      if (isScroll) {
        ...
        var filters = {
          ...
          payDateFrom: filteredValues.payDateFrom,
          payDateTo: filteredValues.payDateTo,
          amountFrom: filteredValues.amountFrom
            ? parseFloat(filteredValues.amountFrom)
            : 0,
          amountTo: filteredValues.amountTo
            ? parseFloat(filteredValues.amountTo)
            : 0,
          from: fromCount,
          count: 20,
          innerFilters: gridFilters,
        ...
        get415ReportInfo(filters)
          .then((response) => {
            ...
            // } else {
            setContribution415Table([
              ...contribution415Table,
              ...response.report415Contributions,
            ]);
  ...
  const exportReportFile = () => {
    ...
    getDownload415Report({ ...filteredValues, selectedColumns: data })
      .then((response) => {
        var blob = new Blob([response], {
          type:
            "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        });
        fileSaver.saveAs(blob, "415ContributionReport.xlsx");
      })
      .catch((error) => {
        console.log("Error while retrieving 415 Contribution Report");
      });
  };
  ...
  return (
    <ManageReportLayout rptName="415 Contribution Limits">
      <div className="w-100 reports-container min-width-200">
                ...
                >
                  <div className="d-flex align-item-center justify-content-between">
                    <div className="d-flex mr-2">
                      ...
                      <div className="d-flex ml-2">
                        <Field
                          ...
                          label="Pay date from "
                          isRequired
                          name={fields.payDateFrom}
                          value={usDateFormat(values[fields.payDateFrom])}
                          direction="bottom"
                          isDatePicker
                          onClear={() => setFieldValue(fields.payDateFrom, "")}
                          popupContent={
                            <DatePicker
                              onDayClick={(value) =>
                                onDaySelected(fields.payDateFrom, value)
                              }
                              value={values[fields.payDateFrom]}
                            />
                          }
                          component={FieldDropSide}
                        />
                      ...
                      <div className="d-flex ml-3">
                        <Field
                          ...
                          label="Pay date to"
                          isRequired
                          name={fields.payDateTo}
                          value={usDateFormat(values[fields.payDateTo])}
                          direction="bottom"
                          isDatePicker
                          onClear={() => setFieldValue(fields.payDateTo, "")}
                          popupContent={
                            <DatePicker
                              onDayClick={(value) =>
                      ...
                      <div className="d-flex ml-3">
                        <Field
                          ...
                          label="Amount from"
                          isRequired
                          name={fields.amountFrom}
                          type="text"
                          autoComplete="off"
                          value={values[fields.amountFrom]}
                          onChange={handleChange}
                          component={FieldInput}
                        />
                      </div>
                      <div className="d-flex ml-3">
                        <Field
                          ...
                          label="Amount to"
                          isRequired
                          name={fields.amountTo}
                          type="text"
                          autoComplete="off"
                          value={values[fields.amountTo]}
                          onChange={handleChange}
                          component={FieldInput}
                        />
                      </div>
                    ...
                    <Button
                      ...
                      style={{ lineHeight: "19px", marginRight: "10%" }}
                      variant="secondary"
                      type="submit"
                    >
                      Search
        ...
        <div className="table-width grid-overflow" style={{ minWidth: "0px" }}>
          <ExcelExport data={contribution415Table}>
            <LoaderWrapper isLoading={dataLoading}>
              ...
              >
                <Grid
                  data={contribution415Table}
                  total={contribution415Table.length}
                  resizable={true}
