...
export const DifferentSourceElectionnSamePartcipantElection = (props) => {
  ...
  useDeepEffect(() => {
    let defaults = !isEmpty(
      autoEnrollment.autoEnrollmentSouceInvestmentElections
    )
      ? [...autoEnrollment?.autoEnrollmentSouceInvestmentElections]
      : [];
  ...
  const deletefunction = (name, index, ind) => {
    ...
    values.autoEnrollmentSouceInvestmentElections[
      index
    ] = values.autoEnrollmentSouceInvestmentElections[index].filter(
  ...
  const Invsum = (index) => {
    let sum1 = 0;
    if (!isEmpty(values.autoEnrollmentSouceInvestmentElections[index]))
      values.autoEnrollmentSouceInvestmentElections[index].map(
  ...
  const AddName = (e, source, index, ind, sName) => {
    if (
      !isEmpty(values.autoEnrollmentSouceInvestmentElections[index]) &&
      !isEmpty(values.autoEnrollmentSouceInvestmentElections[index][ind])
    ) {
        ...
        const sliderPanel = (
          <SliderPanel isOpen={isModalOpen} size="35" showCancel={false}>
            <div className="d-flex justify-content-between align-baseline">
              <div>
                <p className="plan-heading-plan">sfdnjkjb</p>
                  ...
                  <Table>
                    ...
                    <Table.Tbody>
                      {(saveInvestments[index] || []).map((investment, ind) => {
                        const fieldName = `autoEnrollmentSouceInvestmentElections[${index}][${ind}].investmentPercentage`;
                        console.log(
