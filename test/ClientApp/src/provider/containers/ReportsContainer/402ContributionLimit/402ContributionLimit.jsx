...
const GContributionLimit = (props) => {
  const formName = manageReportsFormNames.GCONTRIBUTION_LIMITS;
  ...
  const [g402Table, setG402Table] = useState([]);
  ...
  const apiCall = (data) => {
    setDataLoading(true);
    get402gReportInfo(data)
      .then((response) => {
        setDataLoading(false);
        setG402Table(response.report402GContributions);
  ...
  const onSubmit = (values) => {
    ...
    if (values.companyId !== null) {
      ...
      var filters = {
        ...
        amountFrom: values.amountFrom ? parseFloat(values.amountFrom) : null,
        amountTo: values.amountTo ? parseFloat(values.amountTo) : null,
        from: 1,
        count: 20,
        innerFilters: null,
  ...
  const scrollHandler = (event) => {
    ...
    if (
      ...
      if (isScroll) {
        ...
        setDataLoading(true);
        get402gReportInfo(filters)
          .then((response) => {
            setDataLoading(false);
            if (response === []) {
              setIsScroll(false);
            }
            setG402Table([...g402Table, ...response.report402GContributions]);
          })
          .catch((error) => {
            setDataLoading(false);
          });
  ...
  const exportReportFile = () => {
    ...
    getDownload402gReport({ ...filteredValues, selectedColumns: data })
      .then((response) => {
        var blob = new Blob([response], {
          type:
            "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        });
        fileSaver.saveAs(blob, "402GContributionLimitReport.xlsx");
      })
      .catch((error) => {
        console.log("Error while retrieving 402G Contribution Limit Report");
      });
  };
  ...
  return (
    <ManageReportLayout rptName="402(G) Contribution Limits">
