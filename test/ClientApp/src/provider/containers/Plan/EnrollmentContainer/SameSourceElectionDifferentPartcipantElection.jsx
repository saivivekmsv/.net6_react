...
const SameSourceElectionDifferentPartcipantElection = (props) => {
  ...
  useDeepEffect(() => {
    let defaults = !isEmpty(autoEnrollment.categorywiseInvestments)
      ? [...autoEnrollment?.categorywiseInvestments]
      : [];
    ...
    if (values.investmentBasedOn === 1) {
      if (!isEmpty(autoEnrollment.categorywiseInvestments))
        setInputFields(
          autoEnrollment.categorywiseInvestments.map((x) => ({
            from: x[0].ageFrom,
            to: x[0].ageTo,
          }))
        );
      setSaveInvestments({
        1: uniqBy(
          autoEnrollment.categorywiseInvestments.flatMap((x) => x),
          "investmentName"
        ),
      });
      setAddedInvestments({
  ...
  const deletefunction = (name, index, cellIndex, ind) => {
    ...
    // values.categorywiseInvestments[
    //   cellIndex - 1
    // ] = values.categorywiseInvestments[cellIndex - 1].filter(
    //   (x) => x.investmentName !== name
    // );
    setValues({
      ...values,
      categorywiseInvestments: values.categorywiseInvestments.map((y) =>
        y.filter((x) => x.investmentName !== name)
      ),
    });
  };
  ...
  const Invsum = (index) => {
    let sum1 = 0;
    if (
      !isEmpty(values.categorywiseInvestments) &&
      !isEmpty(values.categorywiseInvestments[index])
    )
      values.categorywiseInvestments[index].map(
        (x) => (sum1 = sum1 + x.investmentPercentage)
      );
    return sum1;
  };
  const AddName = (e, source, index, ind, sName, category) => {
    console.log(values.categorywiseInvestments[0][0]);
    if (
      !isEmpty(values.categorywiseInvestments) &&
      !isEmpty(values.categorywiseInvestments[index]) &&
      !isEmpty(values.categorywiseInvestments[index][ind])
    ) {
      //if(!isEmpty(values.autoEnrollmentSouceInvestmentElections[index].investmentElection[ind])){
      values.categorywiseInvestments[index][ind].investmentName = source;
      const x = sName.split("-");
      console.log(x);
      if (category === 1) {
        values.categorywiseInvestments[index][ind].ageFrom = x[0];
        values.categorywiseInvestments[index][ind].ageTo = x[1];
      } else if (category === 2) {
        values.categorywiseInvestments[index][ind].birthDateFrom = x[0];
        values.categorywiseInvestments[index][ind].birthDateTo = x[1];
      } else if (category === 3) {
        values.categorywiseInvestments[index][ind].retirementDateFrom = x[0];
        values.categorywiseInvestments[index][ind].retirementDateTo = x[1];
      }
      // }
    }
    handleChange(e);
                          ...
                          return (
                            <Table.Tr key={index}>
                              {ageTable.map((item, cellIndex) => {
                                const fieldName = `categorywiseInvestments[${
                                  cellIndex - 1
