...
const MapperSchedDropDown = ({
  ...
  return (
    <div style={{ display: "flex" }}>
      <div className="dropdown2" onMouseOut={() => setOpen(false)}>
        <div className="dropdown2-header" onClick={toggleDropdown}>
          {setLabel()}
          <i className={`fa fa-chevron-down icon ${isOpen && "open"}`}></i>
        </div>
        <div
          className={`dropdown2-body ${isOpen && "open"}`}
          onMouseOver={() => setOpen(true)}
        >
          {items.map((item) => (
            <div
              className="dropdown2-item2"
...
export default MapperSchedDropDown;
