...
import { managePayrollStore, setManagePlanToastInfo } from "../../../contexts";
import {
...
const CreatePayrollListingContainer = (props) => {
  ...
  const UpdatepayrollEmployees = () => {
    UpdatePayrollEmployee(employeeData.filter((_) => _.isPayrollDetail))
      ...
      .catch((error) => {
        console.log("Error while Update Payroll Employee");
      });
  };
  const submitPayroll = () => {
    if (isEmpty(employeeData.filter((_) => _.isPayrollDetail))) {
      setIsPopupOpen(true);
    } else {
      submitCreatePayroll(
    ...
    <LoaderWrapper isLoading={isLoading}>
      ...
      >
        <div className="d-flex justify-content-between align-baseline">
          ...
          <div className="mt-10">
            <Link to={MANAGE_PAYROLL_ROUTES.UPLOADED_FILES_LISTING}>
              Cancel
            </Link>
                        ...
                        {data.isPayrollDetail === true ? (
                          <Table.Tr>
                            ...
                            <Table.Td style={{ width: "10%" }}>
                              {isNull(data.hours) ? "-" : data.hours}
                            </Table.Td>
                            <Table.Td style={{ width: "10%" }}>
                              {isNull(data.planCompensation)
                                ? "-"
                                : data.planCompensation}
                            </Table.Td>
                            <Table.Td style={{ width: "10%" }}>
                              {isNull(data.grossCompensation)
                                ? "-"
          ...
          <Modal.Body style={{ borderTop: "5px solid #f94f50" }}>
            ...
            <div className="d-flex">
              ...
              <div className="remove-text">
                <h4>Warning</h4>
                <p>Add Employees to Payroll to Proceed further</p>
                <br />
